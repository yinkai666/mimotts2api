...
<p align="center">
  将小米 MiMo TTS 封装成爱阅记可直接调用的自建 TTS 服务，提供管理后台、音色配置、配置生成器和合成日志。
</p>
...
- **健康检查**: http://localhost:14678/health
...
1. 在管理后台进入「配置生成器」，选择要使用的本地音色。
2. 填写代理服务地址，例如 `http://localhost:14678` 或你的服务器地址。
3. 如果在「设置」中配置了代理访问 Token，请在生成配置时一并填写；未配置则留空。
4. 复制生成的 JSON 配置。
5. 在爱阅记中进入「设置」→「听书」→「在线语音库管理」→「JSON 输入」，粘贴并保存配置。
